...
namespace Degree_project
{
    public partial class Manager : Form
    {
        ...
        {
            //conn.ConnectionString = "server=localhost;user id=root;password=1234;database=degreeproject";
            //InitializeComponent();
        }
        ...
        private void SpecialityBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            using (MySqlConnection conn = new MySqlConnection(connstr))
            {
                ...
                {
                    DataTable dt_box = new DataTable();
                    sqlCmd = new MySqlCommand("SELECT Degree_Name FROM Degree WHERE Speciality_Name_a = @Speci_a  ", conn);
                    Speciality_a = SpecialityBox.Text;
                    sqlCmd.Parameters.AddWithValue("@Speci_a", Speciality_a);
                    reader = sqlCmd.ExecuteReader();
                    dt_box = new DataTable();
                    dt_box.Columns.Add("Degree_Name", typeof(string));
                    dt_box.Load(reader);
        ...
        private void List_CellContentClick_1(object sender, DataGridViewCellEventArgs e)
        {
